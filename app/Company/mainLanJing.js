...
import Modalcourtanno from './ModalcourtAnno.vue'  //法院公告模态框
import Modalexecuted from './Modalexecuted.vue'  //被执行模态框
import Modaldishonest from './Modaldishonest.vue'  //失信被执行模态框
import Setcolor from './Setcolor.vue'  //自定义拓扑图节点与连线颜色
...
require("./wordCloudBubbles.css");   // 词云图样式表
...
function renderSearchData(queryCompanyName) {
    ...
    $.ajax({
        ...
        url:'Lanjing/function1AccessAjax', //datahub使用
        data:{
            // compNameAjax是接口设定的参数名称
...
function renderWordCloudData(compName){
    ...
    $.post("Lanjing/news/getWordCloud",
        {
...
function getCompanyNewsList(key, compName){
    ...
    $.post("Lanjing/news/getWordCloudList",
        {
            companyName: compName,
            key:key
        },
...
function getThisNews(thisNewsId){
    ...
    $.post("Lanjing/news/getWordCloudNewsDetail",
        {
            key:thisNewsId
        },
        function(data,status){
...
function renderCluster(){
                        ...
                        for(var i=1; i< tmp[id].Node.parentID.length; i++){
                            ...
                            if(tmp[id].hasOwnProperty("children")){
                                ...
                                for(var j=0; j<tmp[id].children.length; j++){
                                    ...
                                    tempObj.role = tmp[id].children[j].role;
    ...
    if(investAndBy){
        investRel = 'invest';
        GetInvestData = handleToTreeData();
        investRel = 'byInvested';
        GetByInvestedData = handleToTreeData();
...
function toggleDisplayColorConfig(){
    ...
    if (this.optionsConfig.colorConfigHandler.length == 1) {
        ...
        $("#setColor").css({'opacity':1, 'top':'51px', 'right': '10px'});
    } else {
...
function smartAnalyse() {
    ...
    // console.log(peopleNameSet);
    // console.log(redundancyPeopleNameSet);
    ...
    redundancyPeopleNodes.forEach(function(node) {
        ...
        postPara += node.properties['key'].substring(node.properties['key'].indexOf('|')+1);
    ...
    function getMergeData(postPara, smartPostPara){
        ...
        $.ajax({
            ...
            success: function(data,status){
                ...
                console.log("newRelationsLinks");
                console.log(newRelationsLinks);
                newmergePeopleNodes(replaceNodesInfo,newRelationsLinks,currentDataStore);
            },
            error: function(){
                console.log("error");
            }
    ...
    function newmergePeopleNodes(replaceNodesInfo,relationsLinks,currentDataStore){
        // console.log("replaceNodesInfo");
        // console.log(replaceNodesInfo);
        // console.log("relationsLinks");
        // console.log(relationsLinks);
        ...
        var tempConcatLinksArr = deepCopyedstartNodeNeededReplaceLinks.concat(relationsLinks);
        // console.log("nodeIdListOfReplaced:");
        // console.log(nodeIdListOfReplaced);
        // console.log("relationsLinks:");
        // console.log(relationsLinks);
        ...
        // 数组去重
        replacedLinksArray = arrUnrepeated(tempConcatLinksArr);
        replacedNodesArray = arrUnrepeated(deepCopyedstartNodeNeededReplaceNodes);
        // console.log("newConcatLinksArr:");
        // console.log(newConcatLinksArr);
        ...
        mergedLinks.forEach(function(n){
            if(!currentDataStore.linksIdSet.has(n.id)){
                currentDataStore.linksIdSet.add(n.id)
            }
        })

        // 过滤掉没有开始和结束节点的连线
        mergedLinks = mergedLinks.filter(function(l) {
            return currentDataStore.nodesIdSet.has(l.startNode)&&currentDataStore.nodesIdSet.has(l.endNode);
        });
    ...
    function mergePeopleNodes(postPara, currentDataStore) {
        $.post("Lanjing/function8ForAjax",
            {
                // nodesInfo: postPara
                companyNodeIdArrText: postcompanyIdStr,
                companyIdPersonName2PersonIdText: smartPostParameter.CompanyIdPersonName2PersonIdText,
                old2NewText: smartPostParameter.Old2NewText,
                personIdArr1Text: smartPostParameter.PersonIdArr1Text
            },
            function(data,status){
...
function renderForce(data, dataSelected, graphConfig, svg) {
    ...
    function loadData(d) {
        ...
        $.post("Lanjing/function10ForAjax",
            {
...
function generateLegencyForCluster(svg) {
    ...
    var lineInvestLegendConfig = {x1: -4, y1: 60, x2: 30, y2: 60, styleClass: {'stroke': '#cd7bdd', 'stroke-width': '2px', 'fill': 'none'}};
    drawLine(legencyG, lineInvestLegendConfig);
    var textInvestLegendConfig = {x: 40, y: 63, styleClass: {'font-size': '10px', 'font-family': 'Microsoft Yahei', 'fill': '#666'}, content: '对外投资'};
    drawText(legencyG, textInvestLegendConfig);

    var lineOwnerLegendConfig = {x1: -4, y1: 80, x2: 30, y2: 80, styleClass: {'stroke': '#019fde', 'stroke-width': '2px', 'fill': 'none'}};
    drawLine(legencyG, lineOwnerLegendConfig);
    var textOwnerLegendConfig = {x: 40, y: 83, styleClass: {'font-size': '10px', 'font-family': 'Microsoft Yahei', 'fill': '#666'}, content: '股东关系'};
    drawText(legencyG, textOwnerLegendConfig);
...
function generateWatermark(svg){
    ...
    for(var i=1; i<=rows; i++){
        if(i<=(rows/2)){
            for(var j=0; j<i; j++){
                var watermarkConfig = {x: j*150, y: 66*i, styleClass: {'opacity': '0.1'}};
...
function createAnimationLayer() {
    ...
    animationLayer.append('text')
        ...
        .style('top', function(){
            if (isFirstLoaded) {
                // isFirstLoaded = false;
                return yPosition - 520 + 'px'; //320
            } else {
                return yPosition - 220 + 'px'; //-20
            }
        })
    ...
    var animationRole = animationLayer.append('div')
        ...
        .style('top', function() { //第一次搜索位置会偏下
            if (isFirstLoaded) {
                isFirstLoaded = false;
                return yPosition - 700 + 'px'; //500
                // return yPosition + 'px'; //500
            } else {
                return yPosition - 400 + 'px'; //200
            }
        });
...
$(document).ready(function(){
    ...
    // console.log("urlCompName:" + urlCompName);
    ...
    //     urlCompName = urlSearch.substring((urlSearch.indexOf("=")+1),(urlSearch.indexOf("&")));
    //     // datahub页面使用 develop.jsp 需要屏蔽
    //     // urlAccountId = urlSearch.substring((urlSearch.indexOf("=",urlSearch.indexOf("accountId"))+1),(urlSearch.indexOf("&",urlSearch.indexOf("accountId"))));
    //     // urlCanCallFlag = urlSearch.substring((urlSearch.indexOf("=",urlSearch.indexOf("canCallFlag"))+1));

    //     if(urlCompName.length>0){
    //         renderSearchData(urlCompName);   
    //     }else{
    //         return;
