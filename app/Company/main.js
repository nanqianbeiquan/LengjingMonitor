...
import Riskanalyse from './components/Riskanalyse.vue'
// import Containerbtp from './Containerbtp.vue'
import Nodeinfo from './components/Nodeinfo.vue'
import Modalinfo from './components/Modalinfo.vue'
import Modalsvg from './components/Modalsvg.vue'
import Modalinvest from './components/Modalinvest.vue'
import Modalnews from './components/Modalnews.vue' //舆情分析词云模态框
// import Modalnewsdetail from './Modalnewsdetail.vue' //新闻详情模态框
import Modalshareholder from './components/Modalshareholder.vue' //股东详情模态框
import Modalinfochange from './components/Modalinfochange.vue' //公司变更详细列表模态框
import Modalcourtanno from './components/ModalcourtAnno.vue'  //法院公告模态框
import Modalexecuted from './components/Modalexecuted.vue'  //被执行模态框
import Modaldishonest from './components/Modaldishonest.vue'  //失信被执行模态框
import Setcolor from './components/Setcolor.vue'  //自定义拓扑图节点与连线颜色

import wordCloud from './modules/d3.layout.cloud' // 词云库文件
import vv from './modules/vv'
// import forceRender from './forceRender'
// import dataParser from './dataParser'
// import simpleExplorableForce from './simpleExplorableForce'
require("bootstrap-webpack");
require("./css/dashboard.css");
// require("./Font-Awesome/less/variables.less");
// require("./style.css"); //原白色主题
// require("./style-black.css");// 原黑色主题
require("./css/style-terminal.css"); // 终端统一主题
require("./css/cluster.css"); // 投资关系集群图样式
import radar from './radarChart'; // 风险分析雷达图库
require("./css/radar-chart.css");   // 雷达图样式
require("./css/wordCloudBubbles.css");   // 词云图样式表
toastr.options.positionClass = "toast-top-center";
/* start 备份main.js记录=====================================================*/
/*
1、2016年07月21日 工商信息接口、司法文书等司法接口全部更新
2、2016年07月29日 棱镜一下后删除叶子节点 
3、2016年08月04日 根据节点权重确定节点大小
4、2016年08月17日 新版将关联风险板块改为异步更新内容
5、2016年09月19月 新版将新增股东与对位投资关系下探功能
...
//== 进入投资与被投资关系圆形树图的标识符
var investAndBy = false;

//== 舆情分析函数实例承载变量
var renderWordCloudInstance = null;
var tags; //舆情分析关键词容器
var wordCloudBubbles = null; // 词云DIV
var wordCloudCoor = {
    x: 0,
    y: 0
}; // 词云节点坐标
var wordCloudRootNodesId; // 存储词云根节点ID
var leftBarWidth; //左边栏宽度
var wordCloudKeyHighlightText;
var wordCloudKeyHighlightTexts = [];
var wordCloudRootNodes; // 存储调出词云的公司

// 存储权限管理字段
var urlCompName;
// var urlAccountId;
// var urlCanCallFlag;
var urlAccountId = "123";
var urlCanCallFlag = 0; //1的权限为可以调第三方接口

// 自定义拓扑图颜色
var colorConfig = {
    nodescolorConfig: ["green", "#009f5d", "#6DCE9E", "#8cc540", "#2456AB", "#017CDC", "#019FDE", "#887ddd", "#cd7bdd", "#ff1244", "#ff5675", "#ff8345", "orange", "#f8bd0b", "#d6dd3a", "#666666", "#cccccc", "#d1d2d4"],
    linkscolorConfig: ["#009f5d", "#8cc540", "#2456AB", "#017CDC", "#019FDE", "#887ddd", "#cd7bdd", "#ff1244", "#ff5675", "#ff8345", "#f8bd0b", "#666", "#d1d2d4"]
}
var changeColorNode = "公司";
var nodesColor = {
    company: '#68BDF6',
    people: '#6DCE9E',
    other: '#ccc',
...
// 切换视图时功能按钮禁用
var allBtnDisable = false;

// 圆形树图实例
var renderClusterInstance;
// 记录圆形树图当前位置信息
var currentOffset_c = { x: 0, y: 0 };
var currentZoom_c = 1.0;
var currentrotateDeg_c = 0; //当前旋转角度
var lastDoZoom = 1;
...
model.data = {
    ...
    searchForm: {
        ...
        tenthSearchBox: '',
        eleventhSearchBox: '',
        moreSearchBox: new Array({ value: "", index: 1 }),
        batchIndex: 1,
        batchMax: { max: 8, error: "批量搜索最多添加8家公司！" }
    },
    ...
    optionsConfig: { //检测是否选中显示全名按钮
        ...
        colorConfigHandler: [] //== 拓扑图节点与连线颜色自定义
    },
    //== 控制侧边栏各导航栏下内容是否显示控制点
    isShowContent: {
        nodesInfo: '',
    ...
    smartAnalyseHighLight: smartAnalyseHighLight,
    allBtnDisable: allBtnDisable, //禁用功能按钮标识
};
// model方法定义
model.methods = {
    ...
    toggleDisplayColorConfig: toggleDisplayColorConfig, //显示或隐藏颜色自定义选项
    ...
    showSelectedLinksType: showSelectedLinksType, //刷新词云图
    closeSetcolorVue: closeSetcolorVue, //刷新词云图
    createNewCompanySearch: createNewCompanySearch,
    delCompanySearch: delCompanySearch,
    getThisNews: getThisNews //舆情新闻列表新闻详情请求
};
// model计算属性定义
model.computed = {
    // 暂时测试屏蔽
    ...
    searchBoxPlaceholder: function () {
        ...
        } else if (this.searchForm.searchMode == 'multi') {
            return '请输入关联企业全称';
            // multiBoxVisible = true;
        } else {
            // multiBoxVisible = false;
...
model.events = {
    ...
    'renderfullScreenCalled': 'fullScreenRender',//窗口化全屏
    ...
    'changeDisplayColorConfig': 'toggleDisplayColorConfig', //显示或隐藏颜色自定义选项
    ...
    'renderGraphByReloadCalled': 'renderGraphByReload',//重新加载操作
    ...
    'showSelectedLinksTypeConfig': 'showSelectedLinksType', //刷新词云图
    'closeSetcolorClicked': 'closeSetcolorVue', //刷新词云图
    'addCompanySearch': 'createNewCompanySearch',
    'delCompanySearchButton': 'delCompanySearch',
    'getThisNewsClicked': 'getThisNews' //舆情新闻列表新闻详情请求事件
};
//end model of mvvm ================================
...
// 将新闻列表标题过滤为20个字符
Vue.filter('summaryNewsListTitle', function (title) {
    // var summaryNewsListTitle = title.substr(0, 15) + '...';
    var summaryNewsListTitle;
    if (title.length >= 15) {
        summaryNewsListTitle = title.substr(0, 15) + '...';
    } else {
        summaryNewsListTitle = title.substr(0, 15);
    }
    return summaryNewsListTitle;
});
...
var vm = new Vue({
    ...
    components: {
        ...
        // Modalnewsdetail, //== 新闻详情模态框
        Modalcourtanno, //==法院公告模态框
        Modaldishonest, //==失信被执行模态框
        Modalexecuted, //==被执行模态框
        Setcolor, //==自定义节点与连线颜色
    },
    data: model.data,
...
function getNodeRiskInfo(dataStore) {
    ...
    if (compNamesStr != '') {
        ...
        $.post(request_url + "Access/relativeRisk",
            {
...
function renderGraphBySearch() {
    ...
    if (searchForm.searchMode == 'single') { //单公司搜索
        ...
        if (!searchForm.firstSearchBox || !searchForm.searchDimension) {
            toastr.error('多个公司名称请用逗号(,)隔开');
            return;
        } else {
            /*==start 新增by zuozhengqi ===================================================================*/
            //将表单中的中文逗号全部替换为中文逗号
            var list = [];
            for (var i = 0; i < searchForm.moreSearchBox.length; i++) {
                var value = searchForm.moreSearchBox[i].value;
                if (value != null && value != "" && value.length > 0) {
                    list.push(value);
                }
            }
            var searchCompanyList = searchForm.firstSearchBox;
            if (list.length > 0) {
                searchCompanyList += "," + list.join(",");
            }

            //var  newFirstSearchBox= searchForm.firstSearchBox.replace(/\，/g, ",")
            var newFirstSearchBox = searchCompanyList.replace(/\，/g, ",")
            //批量公司搜索
...
function fullScreenRender() {
    if (model.data.topologyBlockFullScreen) {
        $("#fullScreenActionBtn").html('<span class="glyphicon glyphicon-resize-full" aria-hidden="true"></span> 全屏');
        ...
        $("#fullScreenActionBtn").html('<span class="glyphicon glyphicon-resize-small" aria-hidden="true"></span> 还原');
        $("body").css({ "overflow": "hidden" });
        $("#svgGraph").css("width", "100%");
        $("#TopologyBlock").css({
            "position": "fixed",
            "z-index": "2000",
            "background": "#ffffff",
    ...
    model.data.topologyBlockFullScreen = !model.data.topologyBlockFullScreen;

    //alert("fullScreenRender");
...
function renderSearchData(queryCompanyName) {
    ...
    //     d3.select('#animationLayer').remove();
    // }
    // console.log("function1ForAjax:"+urlAccountId+":"+urlCanCallFlag);
    var url = request_url + 'Access/function1ForAjax'
    if (isLoacalDebug()) {
        url = request_url + 'Access/function1AccessAjax'
    }
    $.ajax({
        // url:'function1ForAjax', //develop.jsp使用
        url: url, //datahub使用
        //url:request_url+'Access/function1ForAjax', //终端使用
        data: {
            // compNameAjax是接口设定的参数名称
            // sign: urlAccountId, //datahub 权限管理使用
            // ifGetZhongshu: urlCanCallFlag, //datahub权限管理使用
            compNameAjax: queryCompany
        },
        type: 'post',
        timeout: 120000,
        ...
        success: function (data) {
            //初始化圆形树图根节点
            ClusterRootNodes = [];
            ClusterRootNodes.push(queryCompany);
            // results[0].data是返回json对象包含的提取数据入口
            var rawData = data.results[0].data;
            // console.log(rawData);
            // 移除载入动画@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
function renderAssociateSearchData(queryFirstCompany, querySecondCompany, relationshipsDepth) {
    ...
    $.post(request_url + "Access/function2ForAjax",
        {
        ...
        function (data, status) {
            ...
            //初始化圆形树图根节点
            ClusterRootNodes = [];
            ClusterRootNodes.push(queryFirstCompany);
            // results[0].data是返回json对象包含的提取数据入口
            var rawData = data.results[0].data;
            // 移除载入动画@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            d3.select('#animationLayer').remove();
...
function renderListSearchData(queryCompanyList, listRelationshipsDepth) {
    ...
    $.post(request_url + "Access/function3ForAjax",
        {
            // compNameAjax是接口设定的参数名称
        ...
        function (data, status) {
            ...
            //初始化圆形树图根节点
            ClusterRootNodes = [];
            ClusterRootNodes.push(queryCompanyListArray[0]);
            // results[0].data是返回json对象包含的提取数据入口
            var rawData = data.results[0].data;
            // 载入图形
            // 移除载入动画@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
function showSelectedLinksType() {
    console.log(model.data.selectedStatus.selectedLinksTypeList);
...
function getNodeInfo(d) {
    ...
    if (d.labels[0] == 'Person' || d.labels[0] == 'Unknown') { //新增未知节点类型判断
        ...
        // 异步载入工商信息
        getNodeGSData(vm.$data.nodeInfo, d.properties["公司名称"]);
        // 异步载入司法信息
...
//start 点击获取公司司法信息 =====================
function getNodeGSData(nodeInfoOnModel, queryCompanyName) {
    ...
    // 返回数据类型必须设定为json，默认是string字符串
    var dataType = "json";
    $.post(request_url + "Access/gs/companyinfo",
        {
            // compNameAjax是接口设定的参数名称
        ...
        function (data, status) {
            ...
            nodeInfoOnModel.basicInfo["注册资本"] = RegisteredInfo['Registered_Info:registeredcapital'] || '';
...
function getNodeLeaglData(nodeInfoOnModel, queryCompanyName) {
    ...
    // 返回数据类型必须设定为json，默认是string字符串
    var dataType = "json";
    $.post(request_url + "Access/sf/sfws",
        {
            // compNameAjax是接口设定的参数名称
            companyName: queryCompany
        },
        function (data, status) {
            var leaglInfo = data.data['案件信息'];

            // 将节点司法信息挂载到模型上
            nodeInfoOnModel.leaglInfo = leaglInfo.sort(compare("判决时间"));
            // console.log("nodeInfoOnModel:");
...
function getNodeCourtAnnounData(nodeInfoOnModel, queryCompanyName) {
    ...
    $.post("Access/sf/sfOther",
        ...
        function (data, status) {
            ...
            // 将节点开庭公告信息挂载到模型上
            nodeInfoOnModel.courtAnnoun = courtAnnounInfo.sort(compare("bltin:pub_date"));
...
function getNodeExecutedData(nodeInfoOnModel, queryCompanyName) {
    ...
    // 返回数据类型必须设定为json，默认是string字符串
    var dataType = "json";
    $.post(request_url + "Access/sf/sfOther",
        {
            // compNameAjax是接口设定的参数名称
            companyName: queryCompany,
        ...
        function (data, status) {
            ...
            // 将节点被执行信息挂载到模型上
            nodeInfoOnModel.executed = executed.sort(compare("beizhixing:sj"));
...
function getNodeDishonestExecutedData(nodeInfoOnModel, queryCompanyName) {
    ...
    $.post(request_url + "Access/sf/sfOther",
        ...
        function (data, status) {
            ...
            // 将节点被执行信息挂载到模型上
            nodeInfoOnModel.dishonestExecuted = dishonestExecuted.sort(compare("shixin:fbsj"));
...
function exportPngGraph() {
    ...
    //蓝鲸项目截图添加水印
    // if(svgSource == document.getElementById("svgGraph")){
    //     $('#watermark image').css('opacity', '0.1');
    // }
    // saveSvgAsPng.saveSvgAsPng(svgSource, "PNG-Graph.png");
    // setTimeout(function(){
    //     $('#watermark image').css('opacity', '0'); 
    // }, 500);
}
//end 导出png图片 ==========================
...
function exportSvgGraph() {
    ...
    if (document.getElementById("svgclusterGraph") != null) {
        selectSvg = d3.select('#svgclusterGraph');
    } else {
        selectSvg = d3.select('#vizContainer').select('svg');
    }
    d3_save_svg.save(selectSvg.node(), config, true);
    // 蓝鲸项目截图添加水印
    // if(document.getElementById("svgclusterGraph") != null){
    ...
    //     //截图添加水印
    //     $('#watermark image').css('opacity', '0.1');
    //     d3_save_svg.save(selectSvg.node(), config, false);
    //     setTimeout(function(){
    //         d3_save_svg.save(selectSvg.node(), config, true);
    //     }, 500);
    //     setTimeout(function(){
...
function fetchLegalDetail(clickedSuitTitle) {
    ...
    $.post(request_url + "Access/getJudgmentInfo",
        {
...
function toggleDisplayCompanyName() {
    ...
    if (this.optionsConfig.optionsHandler.length == 0) {
        ...
        allCompanyLabels.style('opacity', 0);
...
//== start 词云 ===================================================================
// start 异步获取词云关键词 ------------
function renderWordCloudData(compName) {
    // console.log("wordCloudRootNodes:"+compName);
    //右上角关闭按钮关闭词云
    d3.select(".closeWordCloud")
        .on('click', function () {
            d3.select('.wordCloudBubbles').remove();
            wordCloudBubbles = null;
        })
    //== 每次进入词云时清除新闻列表内容
    vm.$data.companyNewsList.newsList = [];
    //== 每次请求数据前先清除关键词容器内容
    tags = null;
    var dataType = "json";
    //== http://172.16.0.100:8080/lengjing/test/getWordCloud
    $.post(request_url + "Access/news/getWordCloud",
        {
        ...
        function (data, status) {
            ...
            // console.log(data);
            if (d3.select(".closeWordCloud")) {
                if (data.length == 0) {
                    tags = [{ "key": "无新闻", "value": 0.7 }];
                    renderWordCloudInstance = null;
                    renderWordCloudInstance = new renderWordCloud();
                } else {
                    wordCloudKeyHighlightTexts = [];
                    for (var i = 0; i < data.length; i++) {
                        wordCloudKeyHighlightTexts.push(data[i].key);
                    }
                    // console.log("wordCloudKeyHighlightTexts:");
                    // console.log(wordCloudKeyHighlightTexts);
                    tags = data;
                    renderWordCloudInstance = null;
                    renderWordCloudInstance = new renderWordCloud();
...
// start 关键词&&公司名异步获取公司新闻列表 -------------
function getCompanyNewsList(key, compName) {

    var dataType = "json";
    //== http://localhost:8888/WebContent/test/getWordCloudList
    $.post(request_url + "Access/news/getWordCloudList",
        {
            companyName: compName,
            key: key
        },
        function (data, status) {
            // console.log("getWordCloudList:");
            // console.log(data);
            vm.$data.companyNewsList.newsList = arrUnrepeated(data);
            // console.log(vm.$data.companyNewsList.newsList);
        },
        // 返回数据类型必须设定为json
        dataType
    );
...
// end 关键词&&公司名异步获取公司新闻列表 ---------------

// start 获取新闻列表各条新闻详情 ----------------------
function getThisNews(thisNewsId) {
    // console.log(thisNewsId);
    // $("#news-detail-modal").css({"width":winWidth-180, "height":winHeight-50});
    // $("#modal-body-news-detail").css("height",winHeight - 160);
    ...
    var dataType = "json";
    //== http://localhost:8888/WebContent/test/getWordCloudNewsDetail
    $.post(request_url + "Access/news/getWordCloudNewsDetail",
        {
            key: thisNewsId
        },
        function (data, status) {
            // console.log("getWordCloudNewsDetail:");
            // console.log(data);
            vm.$data.newsDetail = data;
            var regKey;
            var regKeyReplaceContent;

            vm.$data.newsDetail.title = data.title;
            var newsDetailContent = data.content;

            for (var i = 0; i < wordCloudKeyHighlightTexts.length; i++) {
                regKey = '';
                regKeyReplaceContent = '';

                regKey = eval('/' + wordCloudKeyHighlightTexts[i] + '/ig');
                if (wordCloudKeyHighlightTexts[i] == wordCloudKeyHighlightText) {
                    regKeyReplaceContent = '<span class=wordCloudKeyHighlightText>' + wordCloudKeyHighlightTexts[i] + '</span>';
                } else {
                    regKeyReplaceContent = '<span class=wordCloudKeyHighlightTexts>' + wordCloudKeyHighlightTexts[i] + '</span>';
                }
                vm.$data.newsDetail.title = vm.$data.newsDetail.title.replace(regKey, regKeyReplaceContent);
                newsDetailContent = newsDetailContent.replace(regKey, regKeyReplaceContent);
            }
            regKey = eval('/' + wordCloudRootNodes + '/ig');
            regKeyReplaceContent = '<span class=wordCloudRootNodes>' + wordCloudRootNodes + '</span>';
            vm.$data.newsDetail.title = vm.$data.newsDetail.title.replace(regKey, regKeyReplaceContent);
            newsDetailContent = newsDetailContent.replace(regKey, regKeyReplaceContent);

            $('.news-detail-title').html(vm.$data.newsDetail.title);
            $('#news-detail-content').html(newsDetailContent);
        },
        // 返回数据类型必须设定为json
        dataType
    );
...
function renderWordCloud(compName) {
    ...
    var svg = d3.select("#wordCloudCont").append("svg")
        .attr("width", width)
        .attr("height", hight);
    ...
    function draw(data, bounds) {
        ...
        text.style("font-family", function (d) {
            ...
            .on('click', function (d) {
                ...
                //获取关键词新闻列表
                getCompanyNewsList(d.text, d.companyName);
                //获取在新闻详情内容中需要高亮显示的关键字文本
                wordCloudKeyHighlightText = d.text;
            })
    ...
    this.update = function () {
        ...
        fontSize = d3.scale['sqrt']().range([8, 16]);
        if (tags.length) {
            // fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]);
            fontSize.domain([0.01, 1]);
        }
...
//== start 切换至圆形树图显示企业股东与对外投资关系 ================
$('#showTreeContainer').click(function () {
    //== 禁用功能按钮
    disableAllBtn();
    $('#treeContainer').css({ 'left': '0px' });
    $("#clusterContainer2")
        .css({ "width": document.getElementById('treeContainer').offsetWidth, "height": document.getElementById('treeContainer').offsetHeight })
    renderClusterInstance = new renderCluster(true);
})
function closeTreeG() {
    //== 恢复功能按钮到上一状态
    enableAllBtn();
    $('#treeContainer').css({ 'left': '5000px' });
    d3.select('#bufferLayer')
        .style('left', '5000px')
        .style('opacity', 0);
    investAndBy = false; //关闭模态框时将投资与被投资标识符置为false
    // 判断是否处于全屏状态
    if (isFullScreen == true) {
        exitFullScreen();
    }
    //删除绘图
    if (d3.select('#clusterContainer2 svg')) {
        d3.select('#clusterContainer2 svg').remove();
    }
}
$('.closeTreeContainer').click(closeTreeG);
$('.fullScreenTreeContainer').click(function () {
    fullScreenTree();
})
// 圆形视图左旋转、右旋转、放大、缩小
$('.rotateLeftBtn').click(function () {
    var rotateDeg = (currentrotateDeg_c - 10) % 360
    renderClusterInstance.repositionGraph(currentOffset_c, undefined, 'drag', rotateDeg);
    currentrotateDeg_c = rotateDeg;
})
...
$('.zoomInBtn').click(function () {
    if (currentZoom_c >= 2.5) {
        currentZoom_c = 2.5;
        // alert('不能再放大了！');
    } else {
        renderClusterInstance._doZoom(0.2);
    }
})
$('.zoomOutBtn').click(function () {
    if (currentZoom_c <= 0.5) {
        currentZoom_c = 0.5;
        // alert('不能再缩小了！');
    } else {
        renderClusterInstance._doZoom(-0.2);
...
var fullScreen = document.getElementById('subTreeContainer');
//modalFullScreen
function fullScreenTree() {
    ...
    if (isFullScreen) {
        $("#clusterContainer2")
            .css({ "width": window.screen.width, "height": window.screen.height })
    } else {
        $("#clusterContainer2")
            .css({ "width": winWidth - document.getElementById('leftSideBar').offsetWidth, "height": winHeight - 51 })
    }
    // if(isFullScreen){
    //     $("#modal-dialog")
    ...
    if (isFullScreen) {
        ...
        renderClusterInstance = new renderCluster();
    } else {
        exitFullScreen();
        renderClusterInstance = new renderCluster();
    }
...
//== start 切换视图时禁用所有功能按钮 ==================
//allBtnDisable   smartAnalyseHighLight
function disableAllBtn() {
    vm.$data.allBtnDisable = allBtnDisable = true;
    // console.log('allBtnDisable:'+allBtnDisable);
}
function enableAllBtn() {
    vm.$data.allBtnDisable = allBtnDisable = false;
    // console.log('allBtnDisable:'+allBtnDisable);
}
...
function renderCluster(first) {
    ...
    investAndBy = true;
    svgWidth = document.getElementById('clusterContainer2').offsetWidth - 20;
    svgHeight = document.getElementById('clusterContainer2').offsetHeight - 20;
    if (svgWidth > svgHeight) {
        radius = (d3.min([svgWidth, svgHeight]) / 2) < 400 ? d3.min([svgWidth, svgHeight]) / 2 : 400;
    } else {
        radius = 316;
    }
    ...
    //== start 将拓扑图中的投资关系处理为圆形树图所需结构数据函数 ======================================================
    function handleToTreeData() {
        ...
        dataStore.nodes.forEach(function (node) {
            ...
            if (ClusterRootNodes.length < 2 && node.properties['公司名称'] == ClusterRootNodes[0]) {
                ClusterRootNodes.push(node.id);
            }
        })
    ...
    function getClusterData() {
        if (InvestRelationshipDataStroe.links.length < 1) {
            closeTreeG();
            toastr.error('树状图至少需要存在一条投资关系！');
            return;
        }
        if (!InvestRelationshipDataStroe.nodesIdSet.has(ClusterRootNodes[1])) {
            closeTreeG();
            toastr.error('请选择一个公司节点！');
            return;
        }
        //== 投资关系集群图所需数据 ================================
        ...
        function convert(source) {
            ...
            for (n in source) {
                ...
                tmp[item.id].role = item.role;
                if (!("children" in tmp[item.id])) tmp[item.id].children = [];

                if (item.id != item.parentID[0]) {
                    for (var i = 0; i < item.parentID.length; i++) {
                        if (tmp[item.parentID[i]]) {
                            tmp[item.parentID[i]].children.push(tmp[item.id]);
                        }
                        else {
                            tmp[item.parentID[i]] = { children: [tmp[item.id]] };
                        }
                    }
                }
            }
            return JSON.parse(JSON.stringify(tmp[parent]));
        }
        return convert(filterClusterData);
    }
    //== end 将拓扑图中的投资关系处理为圆形树图所需结构数据函数 ========================================================

    //== start 处理单投资关系/单被投资关系/投资&&被投资关系 ------------------------
    var GetInvestData = null;
    var GetByInvestedData = null;
    var treeNodes = null;
    if (investAndBy) {
        investRel = 'invest';
        GetInvestData = first ? handleToTreeData() : getClusterData();
        if (InvestRelationshipDataStroe.links.length < 1) {
            return;
        }
        if (!InvestRelationshipDataStroe.nodesIdSet.has(ClusterRootNodes[1])) {
        ...
        GetByInvestedData = first ? handleToTreeData() : getClusterData();
        for (var i = 0; i < GetByInvestedData.children.length; i++) {
            GetInvestData.children.push(GetByInvestedData.children[i]);
        }
        treeNodes = GetInvestData;
        // console.log("GetInvestData:");
        // console.log(GetInvestData);
        // console.log("GetByInvestedData:");
        // console.log(GetByInvestedData);
    } else {
        treeNodes = first ? handleToTreeData() : getClusterData();
    }
    //== end 处理单投资关系/单被投资关系/投资&&被投资关系 --------------------------
    ...
    // var cpTreeNodes = JSON.parse(JSON.stringify(treeNodes))
    traverseTree(treeNodes);
    // traverseTree(cpTreeNodes);
    nodeMaxDepth = Math.max.apply(null, nodedepthAarry); //获取数组最大值
    if (nodeMaxDepth == 0) {
        nodeMaxDepth = 1;
    }
    // console.log("nodedepthAarry:");
    // console.log(nodedepthAarry);
    // console.log("nodeMaxDepth");
    // console.log(nodeMaxDepth);
    function traverseTree(obj) {
        for (var i = 0; i < 2; i++) {
            if (i == 0) {
                ...
                secondIdNum++;
                nodedepth++;
                nodedepthAarry.push(nodedepth - 1);
            } else {
    ...
    var arrowForInvest = drawArrorForCluster(svg, "#cd7bdd", arrowConfigForInvest);
    ...
    var arrowForByInvested = drawArrorForCluster(svg, "#019fde", arrowConfigForByInvested);
    //== end 画箭头-------------------------
    ...
    //      // .size([360, radius-150])
    //      // .separation(function(a, b) { return (a.parent == b.parent ? 1 : 2)/a.depth })
    //     .size([360, (radius-210)*nodeMaxDepth])
    //     .separation(function(a, b) { 
    //         if(a.depth==1 && b.depth==1){
    //             return (a.parent == b.parent ? 1 : 2)/a.depth;
    //         }else{
    //             if(clusterData.hasOwnProperty('children')){
    //                 if(clusterData.children.length <= 40){
    //                     return (a.parent == b.parent ? 0.3 : 2)/a.depth;
    //                 }else if(clusterData.children.length > 40 && clusterData.children.length < 80){
    //                     return (a.parent == b.parent ? 0.6 : 2)/a.depth;
    //                 }else{
    //                     return (a.parent == b.parent ? 1 : 2)/a.depth;
    //                 }
    //             }else{
    ...
    var tree = d3.layout.tree()
        .size([360, (radius - 210) * nodeMaxDepth])
        .separation(function (a, b) {
            // return (a.parent == b.parent ? 0.3 : 1)/a.depth;
            if (a.depth == 1 && b.depth == 1) {
                return (a.parent == b.parent ? 1 : 2) / a.depth;
            } else {
                if (clusterData.hasOwnProperty('children')) {
                    if (clusterData.children.length <= 1) {
                        return (a.parent == b.parent ? 0.2 : 2) / a.depth;
                    } else if (clusterData.children.length > 1 && clusterData.children.length <= 10) {
                        return (a.parent == b.parent ? 0.3 : 2) / a.depth;
                    } else if (clusterData.children.length > 10 && clusterData.children.length <= 15) {
                        return (a.parent == b.parent ? 0.4 : 2) / a.depth;
                    } else if (clusterData.children.length > 15 && clusterData.children.length <= 20) {
                        return (a.parent == b.parent ? 0.5 : 2) / a.depth;
                    } else if (clusterData.children.length > 20 && clusterData.children.length <= 40) {
                        return (a.parent == b.parent ? 0.6 : 2) / a.depth;
                    } else if (clusterData.children.length > 40 && clusterData.children.length <= 60) {
                        return (a.parent == b.parent ? 1.2 : 2) / a.depth;
                    } else if (clusterData.children.length > 60 && clusterData.children.length <= 80) {
    ...
    var clusterLinks = svgClusterG.selectAll(".link-cluster")
        ...
        .style("stroke", function (d) {
            if (d.target.role == "invest") {
                return '#cd7bdd';
            } else if (d.target.role == "byInvested") {
                return '#019fde';
            }
        })
    ...
    var clusterNodes = svgClusterG.selectAll(".node-cluster")
        ...
        .on("mouseover", function (d) {
            ...
            //== 高亮显示关联路径
            thisPathHighLight(d, nodes, clusterLinks, clusterNodesArray, clusterTextArray);
        })
        .on("mouseout", function (d) {
            hideMouseTooltip();
            // if(isFullScreen==true){
        ...
        .on('dblclick', function (d) {
            // console.log('单击节点：');
            // console.log(d.Node.properties["公司名称"]);
            // console.log(d);
            loadData(d);
        })
    ...
    //缩放比例尺-------------------------
    currentOffset_c = { x: 0, y: 0 };
    currentZoom_c = 1.0;
    currentrotateDeg_c = 0; //当前旋转角度
    ...
    //start 动画控制：拖动、缩放 #######
    this.repositionGraph = function (offset, zValue, mode, rotateDeg) {
        // if transition?
        var doTr = (mode == 'move');
        // drag
        if (offset !== undefined && (offset.x != currentOffset_c.x || offset.y != currentOffset_c.y || rotateDeg != currentrotateDeg_c)) {
            ...
            currentOffset_c.x = offset.x;
            currentOffset_c.y = offset.y;
        }
        ...
        allTexts.attr("x", function (d) {
            ...
            .attr('transform', function (d) {
                ...
                if (d.yfixed != undefined) {
                    if (zValue === undefined) {
                        return newDeg < 180 ? 'translate(' + (currentZoom_c - 1) * d.yfixed + ')' : 'translate(' + (currentZoom_c - 1) * d.yfixed + ')rotate(180)';
                    } else {
                        return newDeg < 180 ? 'translate(' + (zValue - 1) * d.yfixed + ')' : 'translate(' + (zValue - 1) * d.yfixed + ')rotate(180)';
                    }
                }
            })
        // .style("font-size", 12*currentZoom_c + "px")
    ...
    // d3.select("#clusterContainer2 svg")
    //     .on("dblclick", function(){
    ...
    function getRootCoord(d) {
        ...
        var newDeg = d.x + currentrotateDeg_c;
        if (currentrotateDeg_c >= 0) {
            if (newDeg >= 360) {
                newDeg -= 360;
            }
        } else {
            if (Math.abs(currentrotateDeg_c) > d.x) {
                newDeg += 360;
            }
        }
    ...
    function dragRotate(d) {
        ...
        if (currentrotateDeg_c >= 360 || currentrotateDeg_c <= -360) {
            currentrotateDeg_c = 0;
        }
        ...
        var rotateDeg = (currentrotateDeg_c + rotateOffset) % 360;
        that.repositionGraph(currentOffset_c, undefined, 'drag', rotateDeg);

        //== 每次旋转结束后更新一次当前旋转角度
        d3.select("#clusterContainer2 svg").on('mouseup', function (e) {
            currentrotateDeg_c = rotateDeg;
            rotateOffset = 0;
        })
        //== 判断旋转方向函数
    ...
    function dragmove(d) {
        var offset = {
            x: currentOffset_c.x + d3.event.dx,
            y: currentOffset_c.y + d3.event.dy
        };
        that.repositionGraph(offset, undefined, 'drag', currentrotateDeg_c);
    }
    // end 拖动图像 ###############

    // start 缩放图像 ###############
    function doZoom(increment) {
        var zoomOffSet = d3.event.scale - lastDoZoom;
        var newZoom = increment === undefined ? (d3.event.scale === lastDoZoom ? zoomScale(currentZoom_c) : zoomScale(currentZoom_c + zoomOffSet)) : zoomScale(currentZoom_c + increment);
        lastDoZoom = d3.event.scale;
        // console.log('d3.event.scale: ' + d3.event.scale);
        // console.log('zoomScale(currentZoom+increment): ' + zoomScale(currentZoom_c+increment));
        if (currentZoom_c == newZoom) {
            return; //no zoom change
        }

        //compute new offset, so graph center wont move
        var zoomRatio = newZoom / currentZoom_c;
        var newOffset = {
            x: currentOffset_c.x * zoomRatio + graphConfig.svgWidth / 2 * (1 - zoomRatio),
            y: currentOffset_c.y * zoomRatio + graphConfig.svgHeight / 2 * (1 - zoomRatio)
        };
        ...
        that.repositionGraph(newOffset, newZoom, 'zoom', currentrotateDeg_c);
    }
    this._doZoom = function (increment) {
        var newZoom = increment === undefined ? d3.event.scale : zoomScale(currentZoom_c + increment);
        // console.log('zoomScale(currentZoom+increment):_ ' + zoomScale(currentZoom_c+increment));
        if (currentZoom_c == newZoom) {
            return; //no zoom change
        }
    ...
    function searchHighLightPathNodes(d, nodes) {
        ...
        if (getSearchNode == 0) {
            ...
            findFatherNode(d, nodes);
            // console.log("thisPathNodes:");
            // console.log(thisPathNodes);
            findChildrenNode(d, nodes);
            return thisPathNodes;
        }
        function findFatherNode(d, nodes) {
        ...
        function findChildrenNode(d, nodes) {
            if ("children" in d) {
    ...
    //== start 高亮显示选择节点所在路径 ----------
    function thisPathHighLight(d, nodes, clusterLinks, clusterNodesArray, clusterTextArray) {
        var pathNodes = searchHighLightPathNodes(d, nodes);
    ...
    function loadData(d) {
        ...
        // 传入公司名称，ajax载入数据
        var queryCompanyExplore = d.Node.properties["公司名称"];
        ...
        InvestRelationshipDataStroe.nodesIdSet.remove(targetCompanyId); //移除目标节点id
        var allOtherNodesId = InvestRelationshipDataStroe.nodesIdSet.values();
        queryInfoContainer = queryInfoContainer.concat(allOtherNodesId);
        InvestRelationshipDataStroe.nodesIdSet.add(targetCompanyId); //加回目标节点id
        ...
        // $.post(request_url+"Access/function10ForAjax",
        //     {
        ...
        //             // 压入links到新数据对象
        //             // 增进合并黑名单验证@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        //             if (!InvestRelationshipDataStroe.linksIdSet.has(l.id)) {
        //                 // 如果出现没有收录的关系，激活更新图表标记
        //                 // enableIsUpdateForce();
        //                 // 压入新关系数据到绘图数据容器
        //                 InvestRelationshipDataStroe.linksIdSet.add(l.id);
        //                 InvestRelationshipDataStroe.links.push(l);
        //             }
        //             // 压入links两端的nodes到新数据对象
        //             // 增进合并黑名单验证@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        //             if (!InvestRelationshipDataStroe.nodesIdSet.has(l.startNode)) {
        //                 InvestRelationshipDataStroe.nodesIdSet.add(l.startNode);

        //                 var sourceNodeFiltedArray = forceExploreData.nodes.filter(function(n) {
        //                     return n.id == l.startNode;
        //                 });
        //                 InvestRelationshipDataStroe.nodes.push(sourceNodeFiltedArray[0]);
        //             }
        //             // 压入links两端的nodes到新数据对象
        //             // 增进合并黑名单验证@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        //             if (!InvestRelationshipDataStroe.nodesIdSet.has(l.endNode)) {
        //                 InvestRelationshipDataStroe.nodesIdSet.add(l.endNode)

        //                 var targetNodeFiltedArray = forceExploreData.nodes.filter(function(n) {
        //                     return n.id == l.endNode;
        //                 });

        //                 InvestRelationshipDataStroe.nodes.push(targetNodeFiltedArray[0]);
        //             }
        //         });
        //         //判断去除游离的点
        //         // 获取所有连线的开始节点与结束节点的id号
        //         var getStartEndNodeIdSet = d3.set();
        //         InvestRelationshipDataStroe.links.forEach(function(l){
        //             if(!getStartEndNodeIdSet.has(l.startNode)){
        //                 getStartEndNodeIdSet.add(l.startNode);
        //             }
        //             if(!getStartEndNodeIdSet.has(l.endNode)){
        ...
        //             }
        //         })
        //         InvestRelationshipDataStroe.nodes = InvestRelationshipDataStroe.nodes.filter(function(n){
        //             if(!getStartEndNodeIdSet.has(n.id)){
        //                 InvestRelationshipDataStroe.nodesIdSet.remove(n.id);
        //             }
        //             return getStartEndNodeIdSet.has(n.id);
        //         })
        //         // console.log('new InvestRelationshipDataStroe');
        //         // console.log(InvestRelationshipDataStroe);

        //         // 更新绘制力图
        //         refreshClusterGraph(InvestRelationshipDataStroe);
        //     },
        //     // 设置ajax返回数据类型为json
        //     dataTypeExplore
        // )
        $.ajax({
            url: 'Access/function10ForAjax', //节点下探使用
            data: {
                nodeIds: postIdsJoined,
                companyStr: 'All'  //更改后增加的参数
            },
    ...
    //end 双击节点载入新数据########
    //== start 更新绘制力图 =============================
    function refreshClusterGraph(InvestRelationshipDataStroe) {
        // console.log('InvestRelationshipDataStroe');
        // console.log(InvestRelationshipDataStroe);
        //== start 处理单投资关系/单被投资关系/投资&&被投资关系 ------------------------
        // 清除上次数据
        rootID = null;
        GetInvestData = null;
        GetByInvestedData = null;
        treeNodes = null;
        if (investAndBy) {
            investRel = 'invest';
            GetInvestData = getClusterData();
            investRel = 'byInvested';
            GetByInvestedData = getClusterData();
            for (var i = 0; i < GetByInvestedData.children.length; i++) {
                GetInvestData.children.push(GetByInvestedData.children[i]);
            }
            treeNodes = GetInvestData;
        ...
        //删除前次搜索绘图
        if (d3.selectAll('.link-cluster')) {
            d3.selectAll('.link-cluster').remove();
        }
        if (d3.selectAll('.node-cluster')) {
            d3.selectAll('.node-cluster').remove();
        }
        ...
        // //总的圆形树图装载点
        // var clusterGraph = svg.append('g').attr('class', 'clusterGraphParent');
        // d3.select('.clusterGraphParent')
        //     .style("transform", "translate(" + currentOffset.x + "px," + currentOffset.y + "px)rotate("+ currentrotateDeg +"deg)")
        //     // .style("transform", "rotate("+currentrotateDeg+"deg)translate(" + currentOffset.x + "px," + currentOffset.y + "px)")
        //     // .style('transform-origin', '50% 50%!important')
        // console.log('currentOffset');
        // console.log(currentOffset);
        // console.log(currentrotateDeg);
        // var svgClusterG = clusterGraph.append("g")
        //     .attr("transform", "translate(" + svgWidth/2 + "," + (svgHeight/2 + 20) + ")");
        ...
        //     })
        //== 绘制连线 ---------------------------
        clusterLinks = svgClusterG.selectAll(".link-cluster")
            .data(links)
            .enter()
            .append("path")
            .attr("class", "link-cluster")
        ...
        //== 绘制节点 ---------------------------
        clusterNodes = svgClusterG.selectAll(".node-cluster")
            .data(nodes)
            .enter()
            .append("g")
            .attr("class", "node-cluster")
        ...
        clusterNodesArray = clusterNodes.append("circle")
            .attr("r", 5)
            .style("fill", function (d) {
                if (d.Node.properties["公司名称"] == ClusterRootNodes[0]) {
                    return "orange";
        ...
        //== 添加节点文字 ------------------------
        clusterTextArray = clusterNodes.append("text")
            .attr("class", "text-cluster")
            .attr("transform", function (d) { return d.x < 180 ? "translate(0)" : "rotate(180)translate(-0)" })
            .attr("dy", "0.3em")
            .attr("x", function (d) {
        ...
        // 旋转视图包裹层clusterGraph
        clusterNodesArray.call(d3.behavior.drag()
            .on('drag', dragRotate)
        );
        clusterNodesArray.on("mousedown", function (d) {
            d3.event.cancelBubble = true;
        ...
        svg.call(d3.behavior.zoom()
            .x(xScale)
            .y(yScale)
            .scale(currentZoom_c) // 设定初始缩放为当前缩放量currentZoom
            .scaleExtent([0.5, 2.5])
            .on('zoom', doZoom)
        );
        ...
        function doZoomOne() {
            //== 每次缩放更新节点半径大小
            links.forEach(function (link) {
                link.source.y = link.source.yfixed * currentZoom_c;
                link.target.y = link.target.yfixed * currentZoom_c;
            })
            that.repositionGraph(currentOffset_c, currentZoom_c, 'zoom', currentrotateDeg_c);
        }
        // end 缩放图像 ###############
    }
    //== end 更新绘制力图 ===============================
...
function closeSetcolorVue() {
    $("#checkColorConfig").click();
}

//start 显示或隐藏颜色自定义选项========
function toggleDisplayColorConfig() {
    // console.log(this.optionsConfig);

    if (this.optionsConfig.colorConfigHandler.length == 1) {
        // console.log('toggleDisplayColorConfig');
        $("#setColor").css({ 'opacity': 1, 'top': winHeight - 110 + 'px', 'right': '10px' });
    } else {
        // console.log("toggleDisplayColorConfig out");
        $("#setColor").css({ 'opacity': 0, 'top': '-1000px', 'right': '-1000px' });
    }
}
//end 显示或隐藏颜色自定义选项==========

//start 自定义函数 ==================
$("#nodeType").find('li').hover(function () {
    // console.log("hover");
    $("#nodeType li").removeClass('selectedLiHover');
    $(this).addClass('selectedLiHover');
}, function () {
    ...
    // console.log("click");
    $("#nodeType li").removeClass('selectedLiHover');
    $("#nodeType li").removeClass('selectedLi');
    $(this).addClass('selectedLi');
    changeColorNode = $(this).text();
    console.log(changeColorNode);
    if (changeColorNode == '默认设置') {
        nodesColor = {
            company: '#68BDF6',
            people: '#6DCE9E',
            other: '#ccc',
...
$("#colorConfig").find('li').hover(function () {
    $(this).css({ 'width': '12px', 'height': '12px' });
}, function () {
    $("#colorConfig li").css({ 'width': '10px', 'height': '10px' });
}).bind('click', function () {
    var getColor = $(this).attr("data-bgcolor");
    // console.log("background-color:"+getColor);
    switch (changeColorNode) {
        case "公司": nodesColor.company = getColor; break;
        case "自然人": nodesColor.people = getColor; break;
        case "其他类型": nodesColor.other = getColor; break;
        case "高风险": nodesColor.highRisk = getColor; break;
        case "中风险": nodesColor.middleRisk = getColor; break;
        case "低风险": nodesColor.lowRisk = getColor; break;
        case "无风险": nodesColor.safe = getColor; break;
        case "投资": nodesColor.invest = getColor; break;
        case "法人": nodesColor.legalRepre = getColor; break;
        case "任职": nodesColor.serve = getColor; break;
        case "默认设置": break;
        default: return;
    }
...
function toggleColor() {
    ...
    // 修改图例颜色
    d3.selectAll(".circleCompanyLegend").style('fill', nodesColor.company);
    d3.selectAll(".circlePersonLegend").style('fill', nodesColor.people);
    d3.selectAll(".circleUnknownLegend").style('fill', nodesColor.other);
    d3.selectAll(".circleHighRiskLegend").style('stroke', nodesColor.highRisk);
    d3.selectAll(".circleNormalRiskLegend").style('stroke', nodesColor.middleRisk);
    d3.selectAll(".circleLowRiskLegend").style('stroke', nodesColor.lowRisk);
    d3.selectAll(".circleSafeLegend").style('stroke', nodesColor.safe);
    d3.selectAll(".lineInvestLegend").style('stroke', nodesColor.invest);
    d3.selectAll(".lineOwnerLegend").style('stroke', nodesColor.legalRepre);
    d3.selectAll(".lineJobLegend").style('stroke', nodesColor.serve);
...
//== start 舆情分析词云气泡 ===============
function createWordCloudBubbles() {
    if (d3.select('.wordCloudBubbles')) {
        d3.select('.wordCloudBubbles').remove();
    }

    var wordCloudBubblesDiv = vv.ini.createWordCloudDiv('wordCloudBubbles');

    return wordCloudBubblesDiv;
}
//== end 舆情分析词云气泡 =================
...
function clearGraphState() {
    ...
    $(".row .main").css("height", $(".row .main").height());
    //关闭词云
    d3.select('.wordCloudBubbles').remove();
    wordCloudBubbles = null;
    //remove svg
...
function resetPanelDataStatus() {
    ...
    vm.$data.nodeInfo.shareholder = '';
    vm.$data.nodeInfo.changed = '';
    vm.$data.nodeInfo.keyPerson = '';
    vm.$data.nodeInfo.courtAnnoun = '';
    vm.$data.nodeInfo.executed = '';
    vm.$data.nodeInfo.dishonestExecuted = '';
...
function smartAnalyseOrNot() {
    ...
    function getMergeData(postPara, smartPostPara) {
        ...
        $.ajax({
            url: request_url + 'Access/function8ForAjax',
            data: {
                companyNodeIdArrText: postPara,
                companyIdPersonName2PersonIdText: encodeURI(smartPostPara.CompanyIdPersonName2PersonIdText),
...
function renderForce(data, dataSelected, graphConfig, svg) {
    ...
    var linksArray = linksG.selectAll(".link")
        ...
        .attr("marker-end", function (d) {
            ...
            var nodeWeight = d.target.weight < 50 ? d.target.weight / 2 : 25;
    ...
    var linksLabelsArray = linksLabelsG.selectAll(".linksLabels")
        ...
        .text(function (d) {
            if (d.type == '任职') {
                if (d.properties['职务'] && d.properties['职务'] != "null") {
                    return d.properties['职务'];
                } else {
                    return '任职';
                }
            } else if (d.type == '投资') {
                if (d.properties['认缴出资'] && d.properties['认缴出资'] != "null") {
    ...
    var nodesArray = nodesG.selectAll(".nodeCircle")
        ...
        .on('dblclick', function (d) {
            ...
            if (d.labels[0] == 'Company') {
                ...
                // force.friction(0.1);
                //获取双击节点的XY坐标的值
                // window.localStorage.dblclickNodeX = d.x;
                // window.localStorage.dblclickNodeY = d.y;
                // console.log(window.localStorage.dblclickNodeX);
                // console.log(window.localStorage.dblclickNodeY);
            }
        })
    ...
    function popRightClickMenu(rightClickMenu, d) {
        ...
        //var removeNodeButtonClip = "<div class='removeNodeButtonWrapper'><button id='removeNodeButton'>删除该节点</button></div><div class='removeNodeButtonWrapper'><button id='removeNodeTreeButton'>修剪该节点</button></div><div class='removeNodeButtonWrapper newsShows'><button id='viewCompanyNews' class='hidden'>查看该企业舆情热词</button></div>";
        ...
        d3.select('#allInvest')
            .on('click', function (p, i) {
                ...
                setTimeout(function () {
                    ...
                    investAndBy = true;
                    // investRel = "invest";
                    fullScreenTree('modalFullScreen'); // 右键查看投资与股东关系时直接进入全屏模式
                    // renderCluster();
    ...
    function getDeleteNodeTreeInfo(node) {
        ...
        $.post(request_url + "Access/function7ForAjax",
            {
                //接口设定的参数名称
                nodeId: nodeId,
    ...
    function repositionGraph(offset, zValue, mode) {
        ...
        allNodes.attr('r', function (d) {
            var nodeWeight = d.weight < 50 ? d.weight / 2 : 25;
            if (zValue >= 1) {
                return 25 + nodeWeight;
            } else {
                return (25 + nodeWeight) * zValue;
            }
        });
...
function generateLegency(svg) {
    ...
    var lineInvestLegendConfig = { x1: -4, y1: 60, x2: 30, y2: 60, class: 'lineInvestLegend', styleClass: { 'stroke': nodesColor.invest, 'stroke-width': '2px', 'fill': 'none' } };
    drawLine(legencyG, lineInvestLegendConfig);
    var textInvestLegendConfig = { x: 40, y: 63, styleClass: { 'font-size': '10px', 'font-family': 'Microsoft Yahei', 'fill': '#666' }, content: '投资关系' };
    drawText(legencyG, textInvestLegendConfig);

    var lineOwnerLegendConfig = { x1: -4, y1: 80, x2: 30, y2: 80, class: 'lineOwnerLegend', styleClass: { 'stroke': nodesColor.legalRepre, 'stroke-width': '2px', 'fill': 'none' } };
    drawLine(legencyG, lineOwnerLegendConfig);
    var textOwnerLegendConfig = { x: 40, y: 83, styleClass: { 'font-size': '10px', 'font-family': 'Microsoft Yahei', 'fill': '#666' }, content: '法人关系' };
    drawText(legencyG, textOwnerLegendConfig);

    var lineJobLegendConfig = { x1: -4, y1: 100, x2: 30, y2: 100, class: 'lineJobLegend', styleClass: { 'stroke': nodesColor.serve, 'stroke-width': '2px', 'fill': 'none' } };
    drawLine(legencyG, lineJobLegendConfig);
    var textJobLegendConfig = { x: 40, y: 103, styleClass: { 'font-size': '10px', 'font-family': 'Microsoft Yahei', 'fill': '#666' }, content: '任职关系' };
...
function generateLegencyForCluster(svg) {
    ...
    var lineInvestLegendConfig = { x1: -4, y1: 0, x2: 30, y2: 0, styleClass: { 'stroke': '#cd7bdd', 'stroke-width': '2px', 'fill': 'none' } };
    drawLine(legencyG, lineInvestLegendConfig);
    var textInvestLegendConfig = { x: 40, y: 3, styleClass: { 'font-size': '0.05rem', 'font-family': 'Microsoft Yahei', 'fill': '#666' }, content: '对外投资' };
    drawText(legencyG, textInvestLegendConfig);

    var lineOwnerLegendConfig = { x1: -4, y1: 20, x2: 30, y2: 20, styleClass: { 'stroke': '#019fde', 'stroke-width': '2px', 'fill': 'none' } };
    drawLine(legencyG, lineOwnerLegendConfig);
    var textOwnerLegendConfig = { x: 40, y: 23, styleClass: { 'font-size': '0.05rem', 'font-family': 'Microsoft Yahei', 'fill': '#666' }, content: '股东关系' };
    drawText(legencyG, textOwnerLegendConfig);
...
//start 蓝鲸项目生成水印 ==================
function generateWatermark(svg) {
    var svgWidth = $('#svgGraph').attr('width');
    var svgHeight = $('#svgGraph').attr('height');
    var diagonal = Math.sqrt(Math.pow(svgWidth, 2) + Math.pow(svgHeight, 2));
    var columns = Math.ceil(svgWidth / 150);
    var rows = Math.ceil(diagonal / 66);

    var watermarkG = svg.append('g')
        .attr('id', 'watermark')
        .attr('transform', 'translate(10, 10)');
    ...
    for (var i = 1; i <= rows; i++) {
        if (i <= (rows / 2)) {
            for (var j = 0; j < i; j++) {
                var watermarkConfig = { x: j * 150, y: 66 * i, styleClass: { 'opacity': '0' } };
                drawPng(watermarkG, watermarkConfig, i, rows);
            }
        } else {
...
function drawPng(container, pngConfig, index, rows) {
    var g = container.append('g')
    g.style('transform', 'translate(' + (-75) + 'px,-30px)');
    if (index > 2) {
        if (index <= rows / 2) {
            if (index % 2) {
                g.style('transform', 'rotate(-45deg)translate(' + (-75 - 75 * (index - 1)) + 'px,-30px)');
            } else {
                g.style('transform', 'rotate(-45deg)translate(' + (-150 - 75 * (index - 2)) + 'px,-30px)');
            }
        } else if (index <= rows / 2 + 6) {
            if (index % 2) {
                g.style('transform', 'rotate(-45deg)translate(' + (-75 - 75 * 4) + 'px,-30px)');
            } else {
                g.style('transform', 'rotate(-45deg)translate(' + (-150 - 75 * 4) + 'px,-30px)');
            }
        } else {
            if (index % 2) {
                g.style('transform', 'rotate(-45deg)translate(' + (-75) + 'px,-30px)');
            } else {
                g.style('transform', 'rotate(-45deg)translate(' + (-150) + 'px,-30px)');
            }
        }
    } else {
        if (index % 2) {
    ...
    var pngConfig = pngConfig || { x: 100, y: 100, styleClass: '' };
    var image = g.append('image')
        .attr('x', pngConfig.x)
        .attr('y', pngConfig.y)
        .attr('width', 150)
        .attr('height', 46)
        .attr('xlink:href', 'js/ljlogo.png');
    if (pngConfig.styleClass) {
        image.style(pngConfig.styleClass);
    }

    if (pngConfig.class) {
        image.attr('class', pngConfig.class);
    }
...
function createAnimationLayer() {
    ...
    var xPositionOffset = (browserWidth - svgWidth) / 2;
    var yPositionOffset = (browserHeight - svgHeight) / 2;

    var xPosition = xPositionOffset + (svgWidth / 2);
    var yPosition = yPositionOffset + (svgHeight / 2);
    // var yPosition = yPositionOffset + ((window.screen.height- 150)/ 2);
    ...
    animationLayer.style('width', browserWidth + 'px').style('height', browserHeight + 'px').style("z-index", "5000");
    ...
    animationLayer.append('text')
        ...
        .style('top', function () {
            if (isFirstLoaded) {
                // isFirstLoaded = false;
                return yPosition - 260 + 'px'; //320
            } else {
                return yPosition - 260 + 'px'; //-20
            }
        })
    ...
    var animationRole = animationLayer.append('div')
        ...
        .style('top', function () { //第一次搜索位置会偏下
            if (isFirstLoaded) {
                isFirstLoaded = false;
                return yPosition - 300 + 'px'; //500
                // return yPosition + 'px'; //500
            } else {
                return yPosition - 300 + 'px'; //200
            }
        });
...
/*== start 添加批量搜索输入框==================================================*/
function createNewCompanySearch() {
    var list = this.searchForm.moreSearchBox;
    var maxModel = this.searchForm.batchMax;
    if (list.length >= maxModel.max) {
        toastr.error(maxModel.error);
    } else {
        list.push({ value: "", index: ++vm.searchForm.batchIndex });
    }
}
// end 添加批量搜索输入框 ====================

/*== start 删除批量搜索输入框==================================================*/
function delCompanySearch(model) {
    //this.searchForm.moreSearchBox.remove(model);
    var list = this.searchForm.moreSearchBox;
    for (var i = 0; i < list.length; i++) {
        if (model.index == list[i].index) {
            list.splice(i, 1);
            //renderGraphBySearch();
            return;
        }
...
$(document).ready(function () {
    // console.log("权限：" + urlCanCallFlag);
    $("#CompanyTopology").click(function () {
        renderGraphByReload();
    })
    // let compName =getQueryStringArgs().hasOwnProperty('compName')?getQueryStringArgs().compName:'';

    // urlCompName = $("#searchCompName").html();
    // if(urlCompName.length > 0 && urlCompName != "null"){
    //     renderSearchData(urlCompName);
    //     return;
    // }else if(compName!=''){
...
function renderGraphByReload() {
    var companyName = vm.searchForm.firstSearchBox = $("#MonitorCompanyName").text().trim();
    if (companyName.length > 0 && companyName != "null") {
        renderSearchData(companyName);
        return;
    } else if (companyName != '') {
        renderSearchData(companyName);
        return;
...
var nodeRegisterIsFirstClick = false;
...
$('#nodeCourtAnnoun').on('click', function () {
    ...
    vm.$data.isShowContent.leaglInfo = isShowContent.leaglInfo;
    $('#collapseOneInfo').collapse('hide');
    $("#courtAnnounNotice").css({ "display": "none" });
    if (nodeShareholderIsFirstClick) {
        $('#collapseTwoInfo').collapse('show');
    } else {
        $('#collapseTwoInfo').collapse('toggle');
...
$('#nodeExecute').on('click', function () {
    ...
    vm.$data.isShowContent.leaglInfo = isShowContent.leaglInfo;
    $('#collapseOneInfo').collapse('hide');
    $("#executeNotice").css({ "display": "none" });
    if (nodeInfoChangeIsFirstClick) {
        $('#collapseTwoInfo').collapse('show');
    } else {
        $('#collapseTwoInfo').collapse('toggle');
...
$('#nodeExecuted').on('click', function () {
    ...
    vm.$data.isShowContent.leaglInfo = isShowContent.leaglInfo;
    $('#collapseOneInfo').collapse('hide');
    $("#executedNotice").css({ "display": "none" });
    if (nodeKeyPersonnelIsFirstClick) {
        $('#collapseTwoInfo').collapse('show');
    } else {
        $('#collapseTwoInfo').collapse('toggle');
...
//== 风险分析面包折叠控制
var radarClickIsFirstClick = false;
...
function oSearchSuggest(inputId, searchSuggestId, inputValue) {
    ...
    function sendKeyWordToBack(keyword) {
        ...
        $.post(request_url + "Access/linkage/data",
            {
                //传递参数
