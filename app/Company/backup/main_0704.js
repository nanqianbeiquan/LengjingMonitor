     ...
     $.post("sf/sfws",
        {
            // compNameAjax是接口设定的参数名称
            companyName: queryCompany,
            columns:'shixin:lasj,shixin:lxqk,shixin:xq_mc,shixin:xqck,shixin:sxjtqk,shixin:fbsj,shixin:xq_mc'
        },
        function(data,status){
            // console.log("data:");
            // console.log(data);
...
function getThisNews(thisNewsId){
    ...
    $.post("news/getWordCloudNewsDetail",
        {
            key:thisNewsId
        },
        function(data,status){
    ...
    setTimeout(function(){
        ...
        $.post("leida",
            ...
            function(data,status){
                ...
                // }
                var radarRawDataObj = {
                    "背景":{value:1, riskLevel: 0},
                    "司法":{value:1, riskLevel: 0},
                    "经营":{value:1, riskLevel: 0},
                    "正面舆情":{value:1, riskLevel: 0},
                    "负面舆情":{value:1, riskLevel: 0},
                }
                if(data.length>0){
                    ...
                    for(var i=0; i<radarRawDataAarry.length; i++){
                        if(radarRawDataAarry[i].indexOf('工商') != -1){
                            radarRawDataObj['背景'].value = +radarRawDataAarry[i].substr(radarRawDataAarry[i].lastIndexOf(':')+1) + 1;
                            ...
                            radarRawDataObj['司法'].value = +radarRawDataAarry[i].substr(radarRawDataAarry[i].lastIndexOf(':')+1)+ 1;
                            ...
                            radarRawDataObj['经营'].value = +radarRawDataAarry[i].substr(radarRawDataAarry[i].lastIndexOf(':')+1)+ 1;
                            ...
                            // radarRawDataObj['正面舆情'].value = Math.abs(+radarRawDataAarry[i].substr(radarRawDataAarry[i].lastIndexOf(':')+1))+ 1;
                            radarRawDataObj['正面舆情'].value = 0 + 1;
                            ...
                            radarRawDataObj['负面舆情'].value = Math.abs(+radarRawDataAarry[i].substr(radarRawDataAarry[i].lastIndexOf(':')+1))+ 1;
                ...
                startRenderRadar(testString, radarRawDataObj);
            },
            dataType
        );
        ...
        function startRenderRadar(str, getRiskLevel){
...
function generateLegency(svg) {
    ...
    var lineInvestLegendConfig = {x1: -4, y1: 140, x2: 30, y2: 140, styleClass: {'stroke': '#666', 'stroke-width': '2px', 'fill': 'none'}};
...
$(document).ready(function(){
    if(location.search.length>0){
        ...
        // console.log("urlCanCallFlag:"+ urlCanCallFlag);
