...
function renderScatter(data){
    ...
    svg.append("g")
        .attr("class", "y axis")
        .call(d3.svg.axis().scale(y).ticks(4).tickSize(0,0).tickFormat(function(d,i){return yScale(i);}).orient("right"));
    ...
    function highLightElement(rootNodeId){
        ...
        data.links.forEach(function(link){
            if(link.startNode == rootNodeId){
                ...
                highNodes.push(link.endNode);
            }else if(link.endNode == rootNodeId){
                highLinks.push(link.id);
...
function matrixTree(data,treeId,second){
    ...
    svg.selectAll("."+nodeClass)
        ...
        .style("fill", function(d) {console.log(color((d.children ? d : d.parent).name));return  color((d.children ? d : d.parent).name); })
        .on('mouseover', function(d){
